...
def _make_setup(side=Side.LONG, sweep_low=44.0, breaker_bottom=45.0) -> LTFSetup:
    ob = OrderBlock(
        ...
        ob_id=(5, 3, 4, "long"), anchor_swing=SwingPoint(2, 200, 45.0, True),
        msb_bar_index=5,
    )
    return LTFSetup(
        ob=ob, side=side, phase=SetupPhase.ENTRY_PENDING,
        sweep_low=sweep_low,
...
def _make_htf_snap() -> HTFStateSnapshot:
    return HTFStateSnapshot(
        ...
        current_range=TradingRange(high=60.0, low=40.0, midpoint=50.0, bias=Bias.BULLISH),
        active_obs=(),
...
class TestTargetPlacement:
    ...
    def test_fixed_rr_long(self):
        htf = _make_htf_snap()
        target = compute_target_price(48.0, 42.0, Side.LONG, "fixed_rr", htf, fixed_rr=3.0)
        # risk = 6, target = 48 + 18 = 66
        assert target == 66.0

    def test_fixed_rr_short(self):
        htf = _make_htf_snap()
        target = compute_target_price(55.0, 60.0, Side.SHORT, "fixed_rr", htf, fixed_rr=3.0)
...
class TestPositionSizing:
    def test_basic_sizing(self):
        size = compute_position_size(
            equity=100000, risk_pct=0.01, entry_price=4500,
            stop_price=4490, point_value=50.0,
        )
    ...
    def test_minimum_size(self):
        size = compute_position_size(
            equity=1000, risk_pct=0.001, entry_price=4500,
            stop_price=4490, point_value=50.0,
        )
    ...
    def test_zero_risk(self):
        size = compute_position_size(
            equity=100000, risk_pct=0.01, entry_price=4500,
            stop_price=4500, point_value=50.0,
        )
        assert size == 0.0
...
class TestDerisk:
    def test_move_to_be(self):
        pos = Position(
            entry_time=100, entry_price=4500, stop_price=4490,
            target_price=4530, side=Side.LONG, size=2.0, risk_points=10.0,
        )
        result = process_derisk(pos, h1_high=4520, h1_low=4505, derisk_mode="move_to_be", trigger_rr=2.0)
    ...
    def test_half_off(self):
        ...
        result = process_derisk(pos, h1_high=4520, h1_low=4505, derisk_mode="half_off", trigger_rr=2.0)
    ...
    def test_not_triggered(self):
        ...
        result = process_derisk(pos, h1_high=4510, h1_low=4505, derisk_mode="move_to_be", trigger_rr=2.0)
    ...
    def test_derisk_on_intrabar_high(self):
        ...
        result = process_derisk(pos, h1_high=4520, h1_low=4495, derisk_mode="move_to_be", trigger_rr=2.0)
        assert pos.derisk_triggered is True
    ...
    def test_already_triggered(self):
        ...
        result = process_derisk(pos, h1_high=4525, h1_low=4505, derisk_mode="move_to_be", trigger_rr=2.0)
        assert result is None
...
class TestEntryExitCollision:
    def test_collision_long(self):
        ...
        assert check_entry_exit_collision(4500, 4490, h1_high=4505, h1_low=4485, side=Side.LONG) is True

    def test_no_collision_long(self):
        # Bar doesn't reach stop
        assert check_entry_exit_collision(4500, 4490, h1_high=4505, h1_low=4495, side=Side.LONG) is False

    def test_collision_short(self):
        assert check_entry_exit_collision(4500, 4510, h1_high=4515, h1_low=4495, side=Side.SHORT) is True

    def test_no_collision_short(self):
        assert check_entry_exit_collision(4500, 4510, h1_high=4505, h1_low=4495, side=Side.SHORT) is False
