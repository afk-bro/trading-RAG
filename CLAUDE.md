...
- `GET /backtests/wfo/{wfo_id}` - Get WFO details
- `GET /backtests/wfo` - List WFO runs with filters
- `POST /backtests/wfo/{wfo_id}/cancel` - Cancel running WFO
...
- `params_hash`: SHA256 for deduplication
...
- `wfo_runs_total{status}`: Counter for WFO runs
- `wfo_run_duration_seconds`: Histogram (1m to 4h buckets)
- `wfo_folds_total{status}`: Counter for fold completions
- `wfo_candidates_eligible`: Gauge for eligible candidate count
- `wfo_best_score`: Gauge for best selection score
...
- `GET /backtests/wfo` - List WFO runs with filters (workspace_id, status)
